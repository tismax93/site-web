...
function updateCarousel() {
    if (carousel) carousel.style.transform = `translateX(-${offset}px)`;
}
...
function moveNext() {
    ...
    if (offset >= maxOffset) {
        ...
        offset = Math.min(offset + cardWidth * 2, maxOffset);
    }
    updateCarousel();
}
...
if (carousel && nextBtn && prevBtn) {
    ...
    if (carouselOuter) {
        carouselOuter.addEventListener('mouseenter', stopAutoScroll);
        carouselOuter.addEventListener('mouseleave', startAutoScroll);
    }
